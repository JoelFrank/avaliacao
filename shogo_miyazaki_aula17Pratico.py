...
Este projeto implementa um agente autônomo para o Mundo de Wumpus usando o algoritmo 
Q-Learning de aprendizado por reforço.

Funcionalidades:
- Ambiente 4x4 com Wumpus, ouro e poços
- Sistema de recompensas (+1000 ouro, -1000 perigos, -1 movimento, -5 brisa)
- Percepções: Brisa (próximo a poços) e Fedor (próximo ao Wumpus)
- Estado do agente inclui posição, posse do ouro e percepções
...
class WumpusWorld:
    ...
    def get_state(self):
        ...
        return tuple(self.agent_pos + [int(self.has_gold), int(breeze), int(stench)])
    ...
    def calculate_reward(self):
        ...
        # Penalidade por estar em célula com brisa (próximo a poços)
        breeze, _ = self.get_percepts()
        if breeze:
            reward -= 5
...
def test_agent(agent, episodes=10):
    ...
    for episode in range(episodes):
        ...
        if env.has_gold and env.agent_pos == [0, 0]:
            successes += 1
            print(f"Teste {episode + 1}: SUCESSO! Recompensa: {total_reward}")
        else:
            print(f"Teste {episode + 1}: Falhou. Recompensa: {total_reward}")
...
if __name__ == "__main__":
    print("=== MUNDO DE WUMPUS - Q-LEARNING ===")
    ...
    print(f"Score médio final: {np.mean(training_scores[-100:]):.2f}")
    print(f"Epsilon final: {trained_agent.epsilon:.3f}")
    print(f"Estados na Q-table: {len(trained_agent.q_table)}")
